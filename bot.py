...
async def obituary(interaction: discord.Interaction, user: discord.Member):
    ...
    context_lines = await fetch_channel_context(
        interaction.channel, user, limit=50, subject_label="THE DECEASED"
    )
...
async def trial(interaction: discord.Interaction, user: discord.Member):
    ...
    context_lines = await fetch_channel_context(
        interaction.channel, user, limit=50, subject_label="THE ACCUSED"
    )
    ...
    prompt = (
        f"You are a thunderously dramatic courtroom narrator with a flair for the theatrical. "
        f"{user.display_name} stands accused before the court.\n\n"
        f"{evidence_note}"
        ...
        f"Write the charge as if a furious prosecutor is reading it aloud to a shocked courtroom. "
        f"Use dramatic language — words like 'brazenly', 'with malicious whimsy', 'in cold blood', "
        f"'to the horror of all present', etc. Make it feel like a real indictment for a completely unhinged crime.\n\n"
        f"Respond with ONLY ONE sentence, formatted EXACTLY like this:\n"
        f"**THE CRIME:** [one dramatically worded sentence describing the ridiculous charge]\n\n"
        f"Be funny, absurd, and theatrical. No real crimes or sensitive topics."
    )

    system = (
        "You are a thunderously dramatic courtroom narrator for a comedy Discord bot. "
        "Output ONLY a single bolded crime charge — nothing else. "
        "The charge must drip with courtroom gravitas and dramatic vocabulary, "
        "but describe a completely absurd and fictional crime. "
