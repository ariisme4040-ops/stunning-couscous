...
async def trial(interaction: discord.Interaction, user: discord.Member):
    ...
    sentences = [
        ...
        "must change their nickname to 'Guilty as Charged' for one week.",
        "owes everyone in the server a virtual high-five.",
        "must post a haiku about their crimes immediately.",
    ]
    sentence_line = (
        f"\n**SENTENCE:** {user.display_name} {random.choice(sentences)}"
        if verdict == "GUILTY" else ""
    )

    description = (
        f"{crime}\n\n"
        f"**JURY:** {jury_text}\n"
        f"**VERDICT:** {verdict}{sentence_line}"
    )
